...
std::vector<double> Stats::getPriceTimeSeries(TimeSeriesType dataType, double samplePeriod)
{
	...
	int timeIndex = 0 ;
	double timeStart = it->timestamp;
	while(it != fullHistory.end())
	{
		double l_ts = it->timestamp;
		if(l_ts >= timeStart + timeIndex * samplePeriod)
		{
			...
			{
				case ASK:
					valueToBeAdded = ((LimitOrders) it->limitOrders[0]).ask / 100.0;
					break;
				case BID:
					valueToBeAdded = ((LimitOrders) it->limitOrders[0]).bid / 100.0;
					break;
				case MID:
					valueToBeAdded = 0.5 * (((LimitOrders) it->limitOrders[0]).ask/100.0
						+ ((LimitOrders) it->limitOrders[0]).bid/100.0);
					break;
				case LAST:
					valueToBeAdded = it->last;
					break;
				case SPREAD:
					valueToBeAdded = ((LimitOrders) it->limitOrders[0]).ask / 100.0
						- ((LimitOrders) it->limitOrders[0]).bid / 100.0;
