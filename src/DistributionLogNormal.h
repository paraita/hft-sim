...
class DistributionLogNormal : public Distribution {
 ...
 DistributionLogNormal(RandomNumberGenerator *a_rng,
		       double a_logmean = 0,
		       double a_logsigma = 1) : Distribution(a_rng), m_mean(a_logmean), m_sigma(a_logsigma), m_LogNormalDistribution(m_rng->getRNG(), boost::lognormal_distribution<>(a_mean, a_sigma)) { };
