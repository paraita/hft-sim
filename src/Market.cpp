...
void Market::setNextActionTime() {	
  int begin = 1;
  if(m_newsServer != NULL) begin = 0;

  /* agent 0 is the news Server */
  for(int i = begin; i <= m_agentCounter; i++) {
    if(i != m_nextActorId) {
      m_NextActionTime[i] -= m_NextActionTime[m_nextActorId];
    }
  }
  
  if(m_nextActorId == 0) m_newsServer->getNextActionTime();
  else m_NextActionTime[m_nextActorId] = m_agents[m_nextActorId]->getNextActionTime();
...
void Market::createAssets(int a_number, int a_tickSize, int a_defaultBid, int a_defaultAsk) {
  for(int i = 1; i <= a_number; i++) {
    m_assetOrderBooks[m_assetCounter+i] = new OrderBook(this,
							m_assetCounter + i,
							a_tickSize,
							a_defaultBid,
							a_defaultAsk);
...
Agent* Market::getAgent(int a_agentIdentifier) {
  if( a_agentIdentifier> m_agentCounter) throw Exception("Agent does not exist");
  return m_agents[a_agentIdentifier];
}

OrderBook* Market::getOrderBook(int a_assetIdentifier) {
  if( a_assetIdentifier> m_assetCounter) throw Exception("Asset does not exist");
...
void Market::notifyExecution(int a_agentIdentifier, int a_orderIdentifier, double a_time, int a_price) {
  ...
  m_agents[a_agentIdentifier]->notifyExecution(a_orderIdentifier,
					       a_time,
					       a_price);
}
...
void Market::notifyPartialExecution(int a_agentIdentifier, int a_orderIdentifier, double a_time, int a_volume, int a_price) {
  ...
  m_agents[a_agentIdentifier]->chargeMarketFees(l_fee);
  m_agents[a_agentIdentifier]->notifyPartialExecution(a_orderIdentifier,
						      a_time,
						      a_volume,
						      a_price);
}
