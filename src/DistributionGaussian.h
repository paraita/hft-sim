...
class DistributionGaussian : public Distribution {
 ...
 DistributionGaussian(RandomNumberGenerator *a_rng,
		      double a_mean = 0,
		      double a_sigma = 1) : Distribution(a_rng), m_mean(a_mean), m_sigma(a_sigma), m_NormalDistribution(m_rng->getRNG(), boost::normal_distribution<>(a_mean, a_sigma)) { };
